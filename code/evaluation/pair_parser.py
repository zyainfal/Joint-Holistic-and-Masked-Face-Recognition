...
class LFW_CFP_AgeDB_PairsParser(PairsParser):
    """The pairs parser for lfw, cfp, agedb.
    """
    def parse_pairs(self):
        test_pair_list = []
        ...
        while line:
            ...
            if len(line_strs) == 3:
                image_index1 = line_strs[0]
                image_index2 = line_strs[1]
                label = line_strs[2]
                if not image_index1.endswith("jpg"):
                    image_index1 = image_index1 + '.jpg'
                if not image_index2.endswith("jpg"):
                    image_index2 = image_index2 + '.jpg'
            else:
                raise Exception('Line error: %s.' % line)
            test_pair_list.append((image_index1, image_index2, label))
            line = pairs_file_buf.readline().strip()
...
class PairsParserFactory(object):
    ...
    def get_parser(self):
        if self.test_set == 'LFW':
            pairs_parser =  LFW_CFP_AgeDB_PairsParser(self.pairs_file)
            ...
            pairs_parser = LFW_CFP_AgeDB_PairsParser(self.pairs_file)
        elif self.test_set == 'CFPFP':
            pairs_parser = LFW_CFP_AgeDB_PairsParser(self.pairs_file)
        elif self.test_set == 'RMFVD':
            pairs_parser = LFW_CFP_AgeDB_PairsParser(self.pairs_file)
