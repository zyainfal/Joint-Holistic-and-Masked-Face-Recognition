...
from loss.face_losses import MagFace
...
class PartialFCAdamW(torch.nn.Module):
    ...
    def __init__(
        ...
        self.weight_mom_exp_avg: torch.Tensor
        self.weight_mom_exp_avg_sq: torch.Tensor
        self.weight_activated: torch.nn.Parameter
        self.weight_activated_mom_exp_avg: torch.Tensor
        self.weight_activated_mom_exp_avg_sq: torch.Tensor
        ...
        if self.sample_rate < 1:
            ...
            self.register_buffer("weight_mom_exp_avg",
                tensor=torch.zeros_like(self.weight))
            self.register_buffer("weight_mom_exp_avg_sq",
                tensor=torch.zeros_like(self.weight))

            self.register_parameter("weight_activated",
                param=torch.nn.Parameter(torch.empty(0, 0)))
            self.register_buffer("weight_activated_mom_exp_avg",
                tensor=torch.empty(0, 0))
            self.register_buffer("weight_activated_mom_exp_avg_sq",
                tensor=torch.empty(0, 0))
    ...
    def sample(self, 
        ...
        self.weight_activated_mom_exp_avg = self.weight_mom_exp_avg[self.weight_index]
        self.weight_activated_mom_exp_avg_sq = self.weight_mom_exp_avg_sq[self.weight_index]
        
        if isinstance(optimizer, torch.optim.AdamW):
            # TODO the params of partial fc must be last in the params list
            optimizer.state.pop(optimizer.param_groups[-1]["params"][0], None)
            optimizer.param_groups[-1]["params"][0] = self.weight_activated
            optimizer.state[self.weight_activated]["exp_avg"] = self.weight_activated_mom_exp_avg
            optimizer.state[self.weight_activated]["exp_avg_sq"] = self.weight_activated_mom_exp_avg_sq
            if "step" not in optimizer.state[self.weight_activated].keys():
                optimizer.state[self.weight_activated]["step"]=0
        else:
            raise
    ...
    def update(self):
        ...
        if self.sample_rate < 1:
            ...
            self.weight_mom_exp_avg[self.weight_index] = self.weight_activated_mom_exp_avg
            self.weight_mom_exp_avg_sq[self.weight_index] = self.weight_activated_mom_exp_avg_sq
    ...
    def forward(
        ...
        if isinstance(self.margin_softmax, MagFace):
            x_norm = torch.norm(embeddings, dim=1, keepdim=True).clamp(self.margin_softmax.l_a, self.margin_softmax.u_a)# l2 norm
            ada_margin = self.margin_softmax.calc_margin(x_norm)
            loss_g = 1/(self.margin_softmax.u_a**2) * x_norm + 1/(x_norm)
            ...
            if self.fp16:
                logits = logits.float()
            logits = self.margin_softmax(logits, ada_margin, labels)
            loss = self.dist_cross_entropy(logits, labels) + self.margin_softmax.lamda*loss_g.mean()
    ...
    def load_state_dict(self, state_dict, strict: bool = True):
        if self.sample_rate < 1:
            ...
            self.weight_activated_mom_exp_avg.zero_()
            self.weight_activated_mom_exp_avg_sq.zero_()
...
class PartialFCSGD(torch.nn.Module):
    """
    https://arxiv.org/abs/2010.05222
    A distributed sparsely updating variant of the FC layer, named Partial FC (PFC).
    When sample rate less than 1, in each iteration, positive class centers and a random subset of
    ...
    def __init__(
        ...
        sample_rate: float
            The rate of negative centers participating in the calculation, default is 1.0.
        """
        super(PartialFCSGD, self).__init__()
        assert (
            distributed.is_initialized()
        ), "must initialize distributed before create this"
        self.rank = distributed.get_rank()
